...
public:
  ...
  bool set_vocabulary(const cv::Mat& dic);

  bool build(const int max_iter, const int dic_size,
             const std::vector<cv::Mat> &desc);

private:
  BowDictionary() = default;
  ~BowDictionary() = default;
  bool compute_bow_dict(std::vector<cv::Mat> desc,
                        int dic_size, int max_iter);
